...
## 能力评估（2026-09-13，五维度诚实打分）

| 维度 | 等级 | 现状 | 差距 |
|---|---|---|---|
| 工具调用 | ★★★★☆ | lly (TTS/转写/文件) + MCP 四端 + 免费模型池调度 | 工具生态 3 个 |
| 学习能力 | ★★☆☆☆ | 三条蒸馏管线 + 88 原子可查 | 查询是子串匹配，非语义理解 |
| 验证闭环 | ★★★★☆ | 确定性 + 统计容差双模式，双实现互证 | 物理实验生态 1 个 |
| 市场/分发 | ★★★☆☆ | 全链贯通 + 前端 + 信任复利 | 网络效应 = 0 |
| agent 自主性 | ★★☆☆☆ | zerostack 接管单任务执行 | 无自主循环（分解/重试/自我改进） |

## 下一轮路线图（按能力杠杆排序）

- **P0 语义检索层** —— 学习能力 ★★→★★★★ 的单一最大杠杆：本地 embedding（bge-small/MiniLM，零边际成本原则）+ sqlite-vec 向量索引，替换 asr_code/kbv 的子串查询。验收：换种问法（"借用检查器为什么报错"）仍命中所有权原子。
- **P1 agent 自主循环** —— zerostack 常驻 + 任务分解 → 多工具编排 → 失败重试 → 结果沉淀为记忆包。从"被调用的工具"升级为"主动干活的助手"。
- **P1 mesh 健壮性** —— 断线重连 + 会话恢复 + 异网 TURN 实测。
- **P2 实验循环自动化** —— 自主设计实验 → 执行 → 统计验证 → 沉淀（物理世界第一性能力）。
- **P2 结算层** —— attest/用量接 studio-billing。

## 已达成（9/9 目标表，2026-09-12 验收）

mpkg 协议双实现互证 / Radxa 全栈 / lilyco 补漏 / lly+CI / cache-node（HTTP 收敛 + WebRTC 跨机 P2P 字节级一致）/ zerostack 采纳 / kbv 机制验收 / 市场（前端+信任复利）/ 信令+数据通道。
