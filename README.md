...
| 视频知识蒸馏管线+格式 spec（ffmpeg→ASR→切片→OCR 验证） | 4 | 4 | 1 | 5 | 📝 **spec 已定稿（spec/kbv-v0.md），管线六原子待 Radxa 侧实现（ffmpeg+whisper 已就绪）** | **P2** |
| 记忆市场注册表 MVP（publish/search/verify/结算） | 5 | 5 | 2 | 5 | 需求再确认：anthropics/skills **175.8k★**（人写技能共享天文级需求）vs 自动沉淀记忆交换全为 0★ 苗子——市场缺口实锤 | **P2** |
